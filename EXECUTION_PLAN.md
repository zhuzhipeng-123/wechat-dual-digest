...
更新：2026-09-19。根据用户最新要求，本版收缩此前“全部功能各做一点”的范围。本文是首版范围与验收依据，不是实现完成记录。

## 1. 第一版到底交付什么

交付一个 Windows 本机双日报工具：配置公众号 → 登录与发现文章 → 抓原文并核对时间 → 生成练习和招聘两份报告 → 每天定时执行 → 用户直接打开 HTML 阅读。
...
重要区别：微信读书搜一搜中的候选搜索/分页是必需步骤；对外提供“公众号搜索 API、通用文章搜索页面”不是。
...
- 正式任务按配置分钟运行，错过不补跑、失败不自动重启整窗。允许已开始的任务跨午夜收尾。
...
保留 `output/YYYY-MM-DD/招聘.html`、`练习.html`。最小一致发布方案：先完成不可变运行目录里的 JSON/图片，再生成绑定该 run_id 和资源的 HTML，最后替换固定 HTML 入口。HTML 对应 JSON 从页面绑定的版本读取，不把连续替换两个独立文件称为事务。每种日报分别发布，互不要求同时成功。
